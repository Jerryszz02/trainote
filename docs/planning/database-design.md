...
| 实体 | 关键字段 | 关系与语义 |
| --- | --- | --- |
| `Workout` | `id`、标题、开始/结束时间、状态、备注、来源 routine 名称快照 | 级联拥有多个有序 `WorkoutExercise` |
| `WorkoutExercise` | 上游动作 ID、中英文名称快照、顺序、记录模式、备注 | 级联拥有力量组；有氧指标直接保存于动作记录 |
| `StrengthSet` | 顺序、重量 kg、次数、完成状态 | 必须属于一个训练动作 |
...
无。正式同步或导出功能进入范围时，需要重新设计数据版本和冲突策略。
